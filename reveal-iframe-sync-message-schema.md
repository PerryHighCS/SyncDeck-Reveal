...
`studentBoundary` — defaults to the title-slide boundary `{ h: 0, v: 0, f: -1 }` when an iframe is promoted to `student`, so status is non-null from startup even before the instructor has progressed. For instructors it remains `null` until an explicit boundary is set. After `clearBoundary`, the value becomes `null` until the next instructor-follow capture or explicit boundary command establishes a new one. When non-null, this reflects the canonical stored boundary for students and the boundary marker currently shown in the instructor storyboard strip. The stored boundary is normalized to `{ h, v: 0, f: -1 }`, while effective forward permission on the top slide at that `h` may be further refined by the instructor's currently released fragment `f` on flat slides.
