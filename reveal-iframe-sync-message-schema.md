...
- Boundary enforcement remains horizontally canonicalized for stored `studentBoundary` status (`{ h, v: 0, f: -1 }`), but the runtime may retain the requested `f` as an exact same-slide lock target on flat slides so students do not reveal additional fragments ahead of the instructor at the boundary `h`.
- Vertical child-slide movement inside a released stack remains locally navigable at the boundary `h` position. Fragment progression on a flat boundary slide is instead tied to the instructor's exact released fragment position.
...
`studentBoundary` — defaults to the title-slide boundary `{ h: 0, v: 0, f: -1 }` when an iframe is promoted to `student`, so status is non-null from startup even before the instructor has progressed. For instructors it remains `null` until an explicit boundary is set. After `clearBoundary`, the value becomes `null` until the next instructor-follow capture or explicit boundary command establishes a new one. When non-null, this reflects the boundary currently enforced for students and the boundary marker currently shown in the instructor storyboard strip. The runtime normally treats this boundary as horizontal-only and canonicalizes it to `{ h, v: 0, f: -1 }`, while optionally retaining an exact fragment lock for flat boundary slides or exact pullback snap-backs behind the scenes.
